...
class SchedulesAPITestCase(unittest.TestCase):
    ...
    def test_schedule_details_endpoint(self) -> None:
        status, _, payload = self.request("GET", "/api/schedules/brief")
        self.assertEqual(status, 200)
    ...
    def test_schedule_log_endpoint(self) -> None:
        status, _, payload = self.request("GET", "/api/schedules/brief/log")
        self.assertEqual(status, 200)
        self.assertEqual(payload["name"], "brief")
    ...
    def test_schedule_status_endpoint(self) -> None:
        status, _, payload = self.request("GET", "/api/schedules/brief/status")
        self.assertEqual(status, 200)
        self.assertEqual(payload["name"], "brief")
        self.assertEqual(payload["status"], "stopped")
    ...
    def test_manual_run_endpoint(self) -> None:
        status, headers, payload = self.request("POST", "/api/schedules/brief/run")
        self.assertEqual(status, 200)
        self.assertEqual(payload["name"], "brief")
        ...
        log_status, _, log_payload = self.request("GET", "/api/schedules/brief/log")
    ...
    def test_unknown_schedule_returns_404(self) -> None:
        status, _, payload = self.request("GET", "/api/schedules/unknown")
        self.assertEqual(status, 404)
        self.assertEqual(payload["error"]["code"], "task_not_found")

    def test_invalid_request_returns_405(self) -> None:
        status, headers, payload = self.request("GET", "/api/schedules/brief/run")
